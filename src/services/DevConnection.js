...
if (!window.THREE) {
  console.warn("DevConnection requires a global THREE object");
}
...
export class DevClient {
  ...
  stream(command) {
    if (command === "INSTANCES") {
      return of({
        data: [{ status: "INJECTED", version: THREE.VERSION }]
      });
    }
  ...
  get(command) {
    if (command === "INSPECTOR") {
      if (!inspector) {
        inspector = new Inspector({ THREE }, emit);
      }
      window.threeInspector = inspector;
      return of(inspector);
    }
